...
class VariationalRewardTrainer(Trainer):
    r"""
    ...
    def __init__(
        ...
        self.kl_scale = args.temperature * (
            1. / self.num_train_examples) * num_cores

        # Prepare kl annealing

        total_batch_size = num_cores * args.per_device_train_batch_size

        steps_per_epoch = self.num_train_examples / total_batch_size

        self.kl_annealing_steps = args.kl_anneal_fraction * steps_per_epoch * args.num_train_epochs
    ...
    def compute_loss(
        ...
        # sample weights
        num_samples = self.train_num_mc_samples if model.training else self.eval_num_mc_samples

        ## [num_samples,output_dim, hidden_dim]
        weight_samples = score_weight_dist.rsample(
            sample_shape=torch.Size([num_samples]))

        # compute reward samples
        rewards_chosen = vmap(F.linear, (None, 0), 0)(chosen_hidden_states,
                                                      weight_samples)
        rewards_rejected = vmap(F.linear, (None, 0), 0)(rejected_hidden_states,
                                                        weight_samples)
        ...
        if model.training:
            ...
            kl_loss = chosen_outputs["kl_loss"]
            if self.kl_annealing_steps > 0:

                kl_anneal_scale = np.minimum(
                    (self.state.global_step + 1) / self.kl_annealing_steps,
                    1.0)
            else:
                kl_anneal_scale = 1.0
            loss += self.kl_scale * kl_anneal_scale * kl_loss

        # if in eval mode, compute binary cross entropy using the marginal
        else:
            logits = torch.stack([rewards_chosen, rewards_rejected])
            marginal_selected_probs = logits.mean(dim=3).softmax(dim=0)[0]
