...
class Normal(torch.distributions.Normal):
    ...
    def __add__(self, inp):
        if isinstance(inp, Normal):
            ...
            return Normal(self.mean + inp.mean,
                          torch.sqrt(torch.clip(new_cov, min=1e-12)))
    ...
    def __matmul__(self, inp):
        ...
        new_cov = self.covariance_weighted_inner_prod(inp.unsqueeze(-3),
                                                      reduce_dim=False)
        return Normal(self.loc @ inp, torch.sqrt(torch.clip(new_cov,
                                                            min=1e-12)))
...
class DenseNormal(torch.distributions.MultivariateNormal):
    ...
    def inverse_covariance(self):
        ...
        return tp(torch.linalg.inv(self.scale_tril)) @ torch.linalg.inv(
            self.scale_tril)

    @property
    def logdet_covariance(self):
        return 2. * torch.diagonal(self.scale_tril, dim1=-2,
                                   dim2=-1).log().sum(-1)
...
class LowRankNormal(torch.distributions.LowRankMultivariateNormal):
    def __init__(self, loc, cov_factor, diag):
        super(LowRankNormal, self).__init__(loc,
    ...
    @property
    def covariance(self):
        return self.cov_factor @ tp(self.cov_factor) + torch.diag_embed(
            self.cov_diag)
    ...
    def logdet_covariance(self):
        ...
        arg1 = tp(
            self.cov_factor) @ (self.cov_factor / self.cov_diag.unsqueeze(-1))
    ...
    def squeeze(self, idx):
        return LowRankNormal(self.loc.squeeze(idx),
