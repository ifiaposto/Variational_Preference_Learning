...
from .utils import SequenceVariationalClassifierOutputWithPast, VariationalPretrainedConfig
...
class VariationalQwen2ForSequenceClassification(Qwen2PreTrainedModel):
    ...
    def __init__(
        ...
        # Set prior  
        self.prior_scale = config.prior_scale * np.sqrt(
            2. / config.hidden_size)
            
        self.tied_mean = config.tied_mean

        # Initialize posterior log scale

        self.dropout_rate = config.dropout_rate

        sigma = np.sqrt(self.dropout_rate / (1 - self.dropout_rate))

        self.score_log_scale = nn.Parameter(torch.ones([1, self.hidden_size]) *
                                            np.log(sigma),
                                            requires_grad=True)
        ...
        if self.covariance_perturb_rank>0:
            print(self.covariance_perturb_rank)
            self.score_covariance_perturb= nn.Parameter(0.01 * torch.randn(self.num_labels, self.hidden_size, self.covariance_perturb_rank)/self.hidden_size) 
    ...
    def forward(
        ...
        hidden_states = transformer_outputs[0] * np.sqrt(
        ...
        kl_loss = gaussian_kl(weight_dist, self.prior_scale**2, self.tied_mean)
    ...
    def score_weight_dist(self):
        ...
        return LowRankNormal(self.score.weight, self.score_covariance_perturb, scale_diag**2)
